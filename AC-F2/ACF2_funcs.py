...
def read_data(path, Labels, unq_appliance, app_to_read, session, instance_range, filter_data = False, linePlot=False, scatterPlot = False, scatterPlot3d = False):
    Data = {}
...
#        
    ...
    n_estimator = np.arange(1, 10)
    clfs = [GaussianMixture(n_components = n, covariance_type='full', init_params='kmeans', random_state = 5).fit(sng) for n in n_estimator]
    bics = pd.DataFrame([clf.bic(sng) for clf in clfs])
    # Finding the optimum clusters
    bics_shift = bics.shift(1) - bics
...
def Gaussian_clustering(sng,feature_to_use, name, components, verbose = True, plot=False):
    ...
    if (plot):
        fig = plt.figure()
        cluster_ax = fig.add_subplot(111, projection='3d')
...
#                                        merge_name.append(Models[unq[sng1]]['Label'] + 'Clt{}'.format(i) + ' + ' + Models[unq[sng2]]['Label'] + 'Clt{}'.format(j) +' + '+ Models[unq[sng3]]['Label'] + 'Clt{}'.format(k) +' + '+ Models[unq[sng4]]['Label'] + 'Clt{}'.format(l))
    ...
    merged_Model[len(merged_Model)] = {'Mean': np.ones((1,3)).flatten(), 'Vars': np.eye(3), 'Wght' : 1.0 , 'Label' : 'GND,'}
...
def prediction(Agg, Merged_Model):
    ...
    X = Agg.as_matrix(columns=['phAngle','power', 'reacPower'])
...
#    ax = fig.add_subplot(111, projection='3d')
#    for app in Data:
#        ax.scatter(d['power'], d['reacPower'], d['S'], marker = '.', c =colors[i] , alpha = alpha, label=app)
#        i += 1 
#    
