...
from ACF2_funcs import f_score, split, kl_divergence
...
#app_to_read = [0,9]
Traning_Data  = read_data(path, Labels, unq_appliance, app_to_read, session, instance_range, filter_data=True, linePlot=False, scatterPlot=False, scatterPlot3d = True)
...
for i in unq_train:  
    ...
    feature_to_use = ['phAngle','power', 'reacPower']
    opt_cluster = gaussian_model_estimation(Traning_Data[i],feature_to_use, i, plot=False)
    print(opt_cluster)
    GMM, Mean, Vars, Wght = Gaussian_clustering(Traning_Data[i],feature_to_use, i, opt_cluster, verbose = False, plot=False)
    Models[i] ={'Mean': Mean, 'Vars': Vars , 'Wght': Wght, 'Label': i}


#%% KL Divergence of models
print('KL- Divergence of Model clusters')
for app in Models:
    print('For Appliance: {}'.format(app))
    for i in range(0, Models[app]['Mean'].shape[0]-1):
        print('Distance between {} and {}'.format(i, i+1))
        print(kl_divergence(Models[app]['Mean'][i], Models[app]['Vars'][i], Models[app]['Mean'][i+1], Models[app]['Vars'][i+1]))
    print('\n')

#%% Creating Super Clusters
aggregation_level = 3  # Defines how many clusters will be added together to create super cluster
Merged_Model = merge_clusters(Models, unq_train, aggregation_level, plot=False)
...
for i in range(0, len(Merged_Model)):
    print(np.linalg.det(Merged_Model[i]['Vars']))
